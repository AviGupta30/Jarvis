  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      while (true) {
        ...
        setMessages((prev) => {
          ...
          if (newMessages[assistantMsgIndex]) {
             newMessages[assistantMsgIndex].content += cleanChunk;
          }
          return newMessages;
        });
      }
      ...
      setMessages((prev) => {
        const newMessages = [...prev];
        if (newMessages[assistantMsgIndex]) {
            newMessages[assistantMsgIndex].content = "Error connecting to Jarvis backend.";
        }
        return newMessages;
      });
    } finally {
  ...
  return (
    <div className="flex h-screen sci-fi-bg text-white font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col relative z-0 p-4 pl-0">
        ...
        <div className="flex-1 flex flex-col glass-panel rounded-2xl overflow-hidden relative shadow-[0_0_30px_rgba(0,243,255,0.05)] border-t border-b border-cyan-500/20 shadow-cyan-500/10">
          ...
          <header className="p-4 bg-[#020611]/80 backdrop-blur-md border-b border-cyan-900/50 flex items-center justify-between z-10 w-full relative">
             ...
             <div className="hidden md:flex items-center space-x-3">
               ...
               <div className="flex items-center space-x-1">
                 <span className="w-1.5 h-1.5 bg-cyan-400 rounded-full animate-pulse shadow-[0_0_5px_#00f3ff]"></span>
                 <span className="w-1.5 h-1.5 bg-cyan-400 rounded-full animate-pulse shadow-[0_0_5px_#00f3ff]" style={{animationDelay: '150ms'}}></span>
                 <span className="w-1.5 h-1.5 bg-cyan-400 rounded-full animate-pulse shadow-[0_0_5px_#00f3ff]" style={{animationDelay: '300ms'}}></span>
               </div>
             </div>
             ...
             <div className="flex items-center space-x-2 bg-cyan-900/20 px-3 py-1 rounded-full border border-cyan-800/50">
               <span className="relative flex h-2 w-2">
                 ...
                 <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500 shadow-[0_0_5px_#34d399]"></span>
               </span>
            ...
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-cyan-700 space-y-6">
                <div className="w-24 h-24 rounded-full border-2 border-cyan-500/30 flex items-center justify-center shadow-[0_0_30px_rgba(0,243,255,0.1)] relative">
                   <div className="absolute inset-0 rounded-full bg-cyan-500/5 blur-xl animate-pulse"></div>
                   <Bot size={48} className="text-cyan-400 opacity-80" />
                   
                   <div className="absolute inset-[-10px] border border-cyan-500/20 rounded-full animate-[spin_8s_linear_infinite] border-t-transparent"></div>
                </div>
            ...
            <div className="w-full mx-auto space-y-6 relative">
              ...
              {isLoading && (
                <div className="flex items-start space-x-4">
                  <div className="glass-panel text-gray-200 rounded-2xl rounded-tl-none border-l-4 border-l-cyan-400 px-6 py-4 flex items-center space-x-3 w-[120px]">
                    <span className="w-2 h-2 bg-cyan-400 rounded-full animate-bounce shadow-[0_0_5px_#00f3ff]"></span>
                    <span className="w-2 h-2 bg-cyan-400 rounded-full animate-bounce delay-100 shadow-[0_0_5px_#00f3ff]"></span>
                    <span className="w-2 h-2 bg-cyan-400 rounded-full animate-bounce delay-200 shadow-[0_0_5px_#00f3ff]"></span>
                  </div>
                </div>
              )}
        ...
        {/* Floating Voice Indicator Bar & Input */}
        <div className="mt-4 flex flex-col items-center w-full relative z-20 pb-4">
           ...
           <form onSubmit={handleSendMessage} className="w-full max-w-4xl relative flex items-center mb-4 group">
             <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/20 via-blue-500/20 to-cyan-500/20 rounded-full blur group-focus-within:opacity-100 opacity-50 transition-opacity"></div>
             ...
             <div className="relative flex w-full bg-[#030a16] border border-cyan-500/40 rounded-full shadow-[0_0_15px_rgba(0,243,255,0.1)] focus-within:border-cyan-400 focus-within:shadow-[0_0_20px_rgba(0,243,255,0.3)] transition-all overflow-hidden items-center pl-6 pr-2 py-2">
                ...
                <div className="w-2 h-2 bg-cyan-500 rounded-full shadow-[0_0_5px_#00f3ff] animate-pulse"></div>
                ...
                <button 
                  ...
                  className="p-2 rounded-full text-cyan-500 hover:text-white hover:bg-cyan-600 transition-all disabled:opacity-50 disabled:hover:text-cyan-500 disabled:hover:bg-transparent flex items-center justify-center bg-cyan-900/30"
                >
                  <Send size={18} />
                </button>
             </div>
           ...
           {/* Voice Indicator matching the mockup */}
           <div className="relative flex items-center justify-center bg-[#020611]/80 backdrop-blur-md border border-cyan-900/60 rounded-full px-6 py-2 shadow-[0_0_15px_rgba(0,243,255,0.1)] gap-4 overflow-hidden">
              <div className="absolute left-0 top-0 h-full w-8 bg-gradient-to-r from-cyan-500/20 to-transparent"></div>
              <div className="absolute right-0 top-0 h-full w-8 bg-gradient-to-l from-cyan-500/20 to-transparent"></div>
              
              <span className="text-xs font-mono text-cyan-500 tracking-widest opacity-70">
